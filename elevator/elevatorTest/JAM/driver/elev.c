...
static void *polling_thread(void *p)
{
	...
	char upOrderString[ORDER_STRING_SIZE];
	char downOrderString[ORDER_STRING_SIZE];
	char stateString[STATE_STRING_SIZE];
	char backupString[BACKUP_STRING_SIZE];
	
	printf("---------------- arrayToString: ----------------\n\n");
	
	printf("a2s: u = %s\n", arrayToString("u", upOrderString));
	printf("a2s: d = %s\n", arrayToString("d", downOrderString));
	printf("a2s: s = %s\n", arrayToString("s", stateString));
	printf("bUps: b = %s\n", arrayToString("b", backupString));
	
	printf("---------------- stringToArray: ----------------\n\n");
	
	stringToArray(upOrderString);
	stringToArray(downOrderString);
	stringToArray(stateString);
	stringToArray(backupString);
	...
	switch (state) {
		...
		case FREE_STATE: elev_set_speed(0);
			if (Q[0].floor == currentFloor) {state = OPEN_STATE; time(&t1); orderCompleted(0);}
			if ((Q[0].floor > currentFloor) && (Q[0].floor != -1)) {state = UP_STATE; currentOrder = Q[0];}
			if ((Q[0].floor < currentFloor) && (Q[0].floor != -1)) {state = DOWN_STATE; currentOrder = Q[0];}
			break;
		
		case OPEN_STATE: time(&t2); elev_set_speed(0); elev_set_door_open_lamp(1);
			if (((t2-t1)> 3) && !obstruction) {state = FREE_STATE; elev_set_door_open_lamp(0);}
			break;
			
		case UP_STATE: elev_set_speed(1000); checkPriority(); 
			if (currentOrder.floor == currentFloor) {elev_set_speed(-150); state = OPEN_STATE; time(&t1); orderCompleted(0); checkPriority();}
			break;
		
		case DOWN_STATE: elev_set_speed(-1000); checkPriority();
			if (currentOrder.floor == currentFloor) {elev_set_speed(150); state = OPEN_STATE; time(&t1); orderCompleted(0); checkPriority();}
			break;
			
		case STOP_STATE: elev_set_speed(0); initQ(); initButtonLamp(0);
			if (!stop) {state = FREE_STATE;}
			break;
		}
