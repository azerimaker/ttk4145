...
class Driver:
    ...
    def setCommandButton(self, channel, value):
        print "channel", channel
        print "value", value
        floor = self.channelToFloor(channel)[0]
        print floor
        if floor == 1:
            self.setChannel(OUTPUT.LIGHT_COMMAND1, value)
        if floor == 2:
            self.setChannel(OUTPUT.LIGHT_COMMAND2, value)
        if floor == 3:
            self.setChannel(OUTPUT.LIGHT_COMMAND3, value)
        if floor == 4:
            self.setChannel(OUTPUT.LIGHT_COMMAND4, value)
